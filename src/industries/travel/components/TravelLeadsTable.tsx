...
interface TravelLeadsTableProps {
  leads: TravelLead[];
  loading: boolean;
  ...
  onRefetch: () => void;
  onViewDetails?: (lead: TravelLead) => void;
  onEditLead?: (lead: TravelLead) => void;
  columnConfig?: any[];
  maskLeads?: boolean;
}

export function TravelLeadsTable({
  leads, loading, selectedLeads, onSelectionChange, owners = [],
  onRefetch, onViewDetails, onEditLead, columnConfig, maskLeads = false,
}: TravelLeadsTableProps) {
  ...
  const handleReminderConfirm = async (date: Date | null) => {
    ...
    setReminderDialogOpen(false); setPendingStatus(null);
  };
  ...
  if (leads.length === 0) return (
    <div className="text-center py-12 text-muted-foreground">
      ...
      <p>No travel leads found. Add your first guest to get started!</p>
    </div>
  );
          ...
          <TableBody>
            {leads.map((lead, index) => (
              <TableRow key={lead.id} className="animate-row-fade-in" style={{ animationDelay: `${index * 30}ms` }} data-state={selectedLeads.has(lead.id) ? 'selected' : undefined}>
                ...
                <TableCell>{lead.travel_date ? format(new Date(lead.travel_date), 'MMM dd, yyyy') : '-'}</TableCell>
                <TableCell>{lead.trip_type ? <Badge variant="outline">{lead.trip_type}</Badge> : '-'}</TableCell>
                <TableCell>{formatCurrency(lead.package_cost)}</TableCell>
