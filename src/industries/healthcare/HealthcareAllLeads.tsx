...
import { Heart, ChevronLeft, ChevronRight, LayoutGrid, Table2 } from 'lucide-react';
...
import { HealthcareLeadsTable } from './components/HealthcareLeadsTable';
import { HealthcareAddLeadDialog } from './components/HealthcareAddLeadDialog';
import { HealthcareUploadLeadsDialog } from './components/HealthcareUploadLeadsDialog';
import { HealthcareAssignLeadsDialog } from './components/HealthcareAssignLeadsDialog';
import { HealthcareEditLeadDialog } from './components/HealthcareEditLeadDialog';
import { HealthcareLeadDetailsDialog } from './components/HealthcareLeadDetailsDialog';
import { useHierarchy } from '@/hooks/useHierarchy';
import { useHealthcareLeads } from './hooks/useHealthcareLeads';
import { HEALTHCARE_DEPARTMENTS } from './config';
...
export default function HealthcareAllLeads() {
  ...
  const defaultColumns = [
    ...
    { id: 'doctor_preference', label: 'Doctor' },
    { id: 'appointment_date', label: 'Appointment' },
    { id: 'treatment_cost', label: 'Treatment Cost' },
    { id: 'status', label: 'Status' },
    { id: 'sales_owner', label: 'Owner' },
    { id: 'notes', label: 'Notes' },
    { id: 'created_at', label: 'Date' },
    { id: 'email', label: 'Email', defaultHidden: true },
    { id: 'phone', label: 'Phone', defaultHidden: true },
    { id: 'age', label: 'Age', defaultHidden: true },
    { id: 'gender', label: 'Gender', defaultHidden: true },
    { id: 'symptoms', label: 'Symptoms', defaultHidden: true },
    { id: 'insurance_provider', label: 'Insurance', defaultHidden: true },
    { id: 'treatment_type', label: 'Treatment Type', defaultHidden: true },
    { id: 'follow_up_date', label: 'Follow-up', defaultHidden: true },
    { id: 'referral_source', label: 'Referral', defaultHidden: true },
  ...
  const columnConfig = (company as any)?.features?.table_configs?.['healthcare_leads'];
  ...
  const handleDeleteLeads = async () => {
    ...
    try {
      ...
      const { error } = await executeInChunks(leadIds, (chunk) =>
        orgClient.from('leads_healthcare' as any).delete().in('id', chunk)
      );
    ...
    <>
      <div className="space-y-4 md:space-y-6 pb-20 md:pb-0">
        <MobileLeadsHeader
          ...
          icon={<Heart className="h-5 w-5 md:h-6 md:w-6 text-primary shrink-0" />}
          ...
          addButton={!isMobile ? (
            <div className="flex items-center gap-2">
              <HealthcareUploadLeadsDialog />
              <HealthcareAddLeadDialog />
            </div>
          ) : null}
        ...
        {isMobile ? (
          <div className="space-y-3">
            {leads.length === 0 ? (
              <div className="text-center py-12 text-muted-foreground"><p>No leads found. Add your first patient lead!</p></div>
            ) : leads.map((lead) => (
          ...
          <Card><CardContent className="pt-6">
            <HealthcareLeadsTable leads={leads} loading={isLoading} selectedLeads={selectedLeads}
      ...
      <HealthcareAssignLeadsDialog open={assignDialogOpen} onOpenChange={setAssignDialogOpen} selectedLeadIds={Array.from(selectedLeads)} onSuccess={() => setSelectedLeads(new Set())} />
      <HealthcareEditLeadDialog open={!!editingLead} onOpenChange={(open) => !open && setEditingLead(null)} lead={editingLead} onSuccess={refetch} />
      <HealthcareLeadDetailsDialog open={!!viewingLead} onOpenChange={(open) => !open && setViewingLead(null)} lead={viewingLead} owners={filterOptions?.owners || []} maskLeads={company?.mask_leads} />

      {isMobile && <FloatingAddButton onClick={() => setAddDialogOpen(true)} />}
      <HealthcareAddLeadDialog open={addDialogOpen} onOpenChange={setAddDialogOpen} />
      <ColumnConfigDialog open={configOpen} onOpenChange={setConfigOpen} tableId="healthcare_leads" defaultColumns={defaultColumns} />
    </>
  );
}
