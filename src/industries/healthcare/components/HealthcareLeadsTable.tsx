...
import { Phone, Mail, MoreHorizontal, Heart, ChevronDown, Check } from 'lucide-react';
...
interface HealthcareLeadsTableProps {
  leads: HealthcareLead[];
  loading: boolean;
  selectedLeads: Set<string>;
  onSelectionChange: (selected: Set<string>) => void;
  owners?: { label: string; value: string }[];
  onRefetch: () => void;
  onViewDetails?: (lead: HealthcareLead) => void;
  onEditLead?: (lead: HealthcareLead) => void;
  columnConfig?: any[];
  maskLeads?: boolean;
}

export function HealthcareLeadsTable({
  leads, loading, selectedLeads, onSelectionChange, owners = [],
  onRefetch, onViewDetails, onEditLead, columnConfig, maskLeads = false,
}: HealthcareLeadsTableProps) {
  ...
  const [viewingHistoryLead, setViewingHistoryLead] = useState<HealthcareLead | null>(null);
  ...
  const handleUpdateField = async (leadId: string, field: string, value: any) => {
    try {
      const { error } = await orgClient.from('leads_healthcare' as any).update({ [field]: value }).eq('id', leadId);
  ...
  const handleReminderConfirm = async (date: Date | null) => {
    if (pendingStatus) {
      try {
        const { error } = await orgClient.from('leads_healthcare' as any)
  ...
  if (leads.length === 0) {
    return (
      <div className="text-center py-12 text-muted-foreground">
        ...
        <p>No healthcare leads found. Add your first patient lead!</p>
      </div>
    );
  }
  ...
  return (
    <>
                ...
                <TableCell>
                  <div>
                    ...
                    <div className="flex items-center gap-2 text-xs text-muted-foreground mt-0.5">
                      ...
                      {lead.phone && <span className="flex items-center gap-1"><Phone className="h-3 w-3" /><MaskedValue value={lead.phone} type="phone" enabled={maskLeads} /></span>}
                    </div>
                    {lead.age && <div className="text-xs text-muted-foreground">{lead.age} yrs{lead.gender ? `, ${lead.gender}` : ''}</div>}
                  </div>
                </TableCell>
                <TableCell>{lead.department ? <Badge variant="outline">{lead.department}</Badge> : '-'}</TableCell>
                <TableCell>{lead.condition || '-'}</TableCell>
                <TableCell>{lead.doctor_preference || '-'}</TableCell>
                <TableCell>{lead.appointment_date ? format(new Date(lead.appointment_date), 'MMM dd, yyyy') : '-'}</TableCell>
                <TableCell>{formatCurrency(lead.treatment_cost)}</TableCell>
                <TableCell>
                  <DropdownMenu>
      ...
      {pendingStatus && (
        <StatusReminderDialog open={reminderDialogOpen} onOpenChange={setReminderDialogOpen} status={pendingStatus.status}
          onConfirm={handleReminderConfirm} onCancel={() => { setReminderDialogOpen(false); setPendingStatus(null); }} />
      )}
      {viewingHistoryLead && (
        <LeadHistoryDialog open={!!viewingHistoryLead} onOpenChange={(open) => !open && setViewingHistoryLead(null)} lead={viewingHistoryLead as any} />
      )}
    </>
  );
}
