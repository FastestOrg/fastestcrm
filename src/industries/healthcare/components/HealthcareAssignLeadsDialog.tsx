...
export function HealthcareAssignLeadsDialog({ open, onOpenChange, selectedLeadIds, onSuccess }: HealthcareAssignLeadsDialogProps) {
    ...
    try {
      ...
      await Promise.all(notifs.map(({ userId, role }) =>
        notificationService.createNotification({
          userId, title: 'New Healthcare Leads Assigned',
    ...
    <div className="space-y-2">
      ...
      {checked && (
        <Select value={value} onValueChange={onChange}>
          ...
          <SelectContent>{profiles.map(p => <SelectItem key={p.id} value={p.id}>{p.full_name || p.email || 'Unknown'}</SelectItem>)}</SelectContent>
        </Select>
      )}
    </div>
  );
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[450px]">
        <DialogHeader><DialogTitle>Assign Healthcare Leads</DialogTitle></DialogHeader>
