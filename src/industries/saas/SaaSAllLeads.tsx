...
export default function SaaSAllLeads() {
  ...
  const { data: filterOptions } = useQuery({
    queryKey: ['saasLeadsFilterOptions', company?.id, canViewAll, accessibleUserIds, hierarchyLoading],
    queryFn: async () => {
  ...
  const handleDeleteLeads = async () => {
    ...
    try {
      const { error } = await supabase.from('leads_saas' as any).delete().in('id', Array.from(selectedLeads));
  ...
  const handleStatusChange = async (leadId: string, newStatus: string) => {
    try {
      const { error } = await supabase.from('leads_saas' as any).update({ status: newStatus }).eq('id', leadId);
      if (error) throw error;
