...
export function InsuranceAssignLeadsDialog({ open, onOpenChange, selectedLeadIds, onSuccess }: Props) {
  ...
  const handleAssign = async () => {
    ...
    try {
      ...
      await Promise.all(notifs.map(({ userId, role }) => notificationService.createNotification({
        userId, title: 'New Insurance Leads Assigned',
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[450px]">
        <DialogHeader><DialogTitle>Assign Insurance Leads</DialogTitle></DialogHeader>
